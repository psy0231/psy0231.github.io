...
# 4  기초문법(클래스와 메서드)
- 클래스: 속성(상태)과 기능(동작)의 정의.  
- 객체: 클래스의 정의로 생성된 대상이다. 
- 객체의 특징인 속성은 변수로, 객체가 할 수 있는 일인 기능은 메서드로 구현.  
즉, 객체는 클래스의 정의대로 변수와 메서드의 묶음으로 이루어져있다.
  - ex1) 객체가 주식종목 이라면 stock_name(종목명), price(현재가), rate(수익률)과 같은 속성은 변수로 구현하고 print(출력), change_price(가격 변경)과 같은 기능(동작)은 메서드로 구현
  - ex2) 객체가 자전거라면 바퀴의 크기, 색깔 같은 속성은 변수로 구현하고 전진, 방향 전환, 정지 같은 기능(동작)은 메서드로 구현
...
- from 모듈 import [module, class, mtd] as 별칭
-  __name__? => 현재 모듈의 이름을 담고 있는 내장 변수
  - 실제 실행되는건 __main__이라 되고  
  그 외 불러오는 py는 __name__에서 다 다름
