...
### 개요
- 쿼리 식은 LINQ 사용 데이터 소스에서 데이터를 쿼리하고 변환하는 데 사용. 
  - ex) 단일 쿼리로 SQL 데이터베이스에서 검색하고 XML 출력.
- 관계형 데이터베이스에는 다양한 형식의 데이터 소스에 대해 서로 다른 언어가 필요했는데  
...
- 컴파일 타임에 쿼리 식은 C#에 명시된 규칙에 따라 표준 쿼리 연산자 메서드 호출으로 변환된다.  
  ...
  - Count 또는 Max와 같은 일부 쿼리 작업은  
  해당하는 쿼리 식 절이 없으므로 메서드 호출로 표현해야 한다.  
  메서드 구문을 다양한 방법으로 쿼리 구문에 조합할 수 있다.  
  자세한 내용은 쿼리 구문과 메서드 구문 비교를 참조.
  - 쿼리 식은 쿼리가 적용되는 형식에 따라 식 트리 또는 대리자로 컴파일될 수 있다.  
...
- **Deferred Execution**
  ...
  - 쿼리 변수 자체는 쿼리 결과를 저장하지 않으므로 원하는 만큼 자주 실행할 수 있다.  
    - 예를 들어 애플리케이션에서 지속적으로 업데이트되는 데이터베이스가 있을 때  
    이 애플리케이션에서 최근 데이터를 검색하는 쿼리를 작성하고  
    이를 일정 간격을 두고 반복적으로 실행하여 매번 다른 결과를 검색할 수 있다.
...
- **Forcing Immediate Execution**
  ...
  - 또한 이러한 유형의 쿼리는 IEnumerable 컬렉션이 아니라 단일 값을 반환한다.  
  - 배열에서 짝수의 개수를 반환하는 예.
    ```c#
    var evenNumQuery =
        from num in numbers
    ...
    ToList 또는 ToArray를 호출하여 단일 컬렉션 개체에서 모든 데이터를 캐시할 수도 있다.
