...
T 형식을 반환하는 모든 메서드는 Task\<T>를 반환하는 비동기 메서드가되고  
...
이벤트 처리기는 자연스럽게 void를 반환하므로  
비동기 메서드는 void를 반환하므로 비동기 이벤트 처리기를 사용할 수 있음.  
...
비동기 void 메서드를 사용하면 Task 개체가 없으므로  
비동기 void 메서드에서 throw 된 모든 예외는  
비동기 void 메서드가 시작될 때 활성화 된 SynchronizationContext에서 직접 발생합니다.  
다음은 비동기 void 메서드에서 throw 된 예외를 자연스럽게 포착 할 수 없음을 보여줍니다.
...
SynchronizationContext에서 직접 예외를 발생시킵니다.  
...
이벤트 처리기 인 메서드 (예 : ICommand.Execute 구현)가 포함됩니다.
...
콘솔 응용 프로그램은이 솔루션을 완전히 따를 수 없다.  
Main 메서드가 비동기 인 경우  
완료되기 전에 반환되어 프로그램이 종료 될 수 있다.  
다음은 지침에 대한이 예외로 콘솔 응용 프로그램의 Main 메서드는  
코드가 비동기 메서드에서 차단 될 수 있는 몇 안되는 상황 중 하나임.
...
이 메소드가 GUI 컨텍스트에서 호출되면 GUI 스레드를 차단함.  
ASP.NET 요청 컨텍스트에서 호출되면  
현재 ASP.NET 요청 스레드를 차단.  
비동기 코드는 동기적으로 차단되지 않는 경우 가장 잘 작동함.  
...
예상보다 빨리 완료 될 수 있으며,  
기다리기 전에 완료되는 반환 된 작업을 정중하게 처리해야합니다.  
#6 은 수정 된 예.
