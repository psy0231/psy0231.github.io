...
UI thread는 반는하기때문에 해당 프로세스를 멈추거나 하는  
다른 UI작업이 가능해짐.
...
async키워드 를 사용하여 정의하는 비동기 메서드를 비동기 메서드 라고 한다.
...
- 위 예제 순서
    ...
    2. GetUrlContentLengthAsync는 HttpClient 인스턴스를 만들고 GetStringAsync 비동기 메서드를 호출하여 웹 사이트의 내용을 문자열로 다운로드.
    ...
    메서드가 문자열을 확인할 때까지 해당 값을 계산할 수 없습니다.
        ...
        따라서 GetUrlContentLengthAsync는 await 연산자를 사용해서 해당 프로세스를 일시 중단하고 GetUrlContentLengthAsync를 호출한 메서드에 제어 권한을 양도합니다.  
...
await 식이 없는 경우 컴파일러 오류가 발생하지는 않습니다.  
