...
리소스를 차단하지 않기 위해 GetStringAsync는 해당 호출자인 GetUrlContentLengthAsync에 제어 권한을 양도합니다.
    ...
    GetStringAsync를 호출하는 지속적인 프로세스를 나타냅니다.  

    작업은 작업이 완료 될 때 실제 문자열 값을 생성하겠다는 약속과 함께  
    GetStringAsync 호출에 대한 진행중인 프로세스를 나타냅니다.
4. getStringTask가 아직 대기되지 않았으므로  
즉, 아직 await가 쓰이지 않았으므로  
GetUrlContentLengthAsync가 GetStringAsync의 최종 결과에 무관한 다른 작업을 계속할 수 있습니다.  
이 경우는 DoIndependentWork()이 수행되는것.
5. DoIndependentWork는 작업을 수행하고 호출자에게 반환하는 동기 메서드입니다.
6. GetUrlContentLengthAsync에는 더이상 getStringTask의 결과 없이 수행할 수 있는 작업이 없음.  
...
메서드가 문자열을 확인할 때까지 해당 값을 계산할 수 없음.
    ...
    참고  
    GetUrlContentLengthAsync가 await하기 전에 GetStringAsync(및 getStringTask)가 완료되면  
...
GetStringAsync를 호출할 경우 문자열 결과가 예상대로 반환되지 않습니다. 
...
대기 이벤트 처리기를 다시 시작할 수 있습니다.  
    ...
    비동기 메서드는 작업이 일시 중단될 때 반환됩니다.(3, 6단계)  
...
- await 연산자 
    ...
    - 그런 다음, await 연산자가 getStringTask에서 string 결과를 검색.
...
비동기 메서드의 await 식은 대기한 작업이 실행되는 동안 현재 스레드를 차단하지 않음.  
...
비동기 메서드 내에서 await 연산자는 호출에서 다른 비동기 메서드로 전환되는 작업에 적용.
- 메서드에 TResult 형식의 피연산자를 지정하는 return 문이 포함되어 있을 경우  
Task\<TResult>를 반환 형식으로 지정합니다.
...
비동기 이벤트 처리기는 비동기 프로그램의 시작점 역할을 하는 경우가 많다.
...
작업 결과를 반환하거나 예외가 발생하지 않음을 알려야 한다..
...
- return 형식
    ...
    - void 
        ...
        성공적인 완료 또는 오류 조건을 보고하기 위해 다른 메커니즘을 구현해야하므로  
    ...
    - C#7.0부터는 GetAwaiter (System.Threading.Tasks.ValueTask\<TResult>)
        ...
        - C# 7.0부터 GetAwaiter 메서드가 있는 다른 형식(일반적으로 값 형식)을 반환하여  
        성능이 중요한 코드 섹션에서 메모리 할당 최소화.
...
- ex
    ...
    작업이 완료 될 때까지 await 연산자는 DownloadDocsMainPageAsync 메서드를 일시 중단.  
    ...
    비동기 작업의 결과가 필요할 때까지 실행됩니다.  
    GetByteArrayAsync가 모든 바이트를 가져 오면  
    나머지 DownloadDocsMainPageAsync 메서드가 평가됩니다.   
    그 후 나머지 Main 메서드가 평가됩니다.
    - C# 7.1부터 애플리케이션 진입점인 Main 메서드는  
    Task 또는 Task\<int>를 반환하여 해당 본문에서 await 연산자를 사용할 수 있습니다.  
    이전 C# 버전에서는 Main 메서드가 비동기 작업이 완료될 때까지  
    대기하는지 확인하기 위해 해당 비동기 메서드에서 반환되는  
    Task\<TResult> 인스턴스의 Task\<TResult>.Result 속성 값을 검색할 수 있습니다.  
...
- Main 메서드의 await 연산자
C# 7.1부터 애플리케이션 진입점인 Main 메서드는 Task 또는 Task\<int>를 반환하여  
해당 본문에서 await 연산자를 사용할 수 있습니다.  
이전 C# 버전에서는 Main 메서드가 비동기 작업이 완료될 때까지 대기하는지 확인하기 위해  
해당 비동기 메서드에서 반환되는 Task\<TResult> 인스턴스의  
Task\<TResult>.Result 속성 값을 검색할 수 있습니다.  
값을 생성하지 않는 비동기 작업의 경우 Task.Wait 메서드를 호출할 수 있습니다.  
...
- 네트워크에서 데이터 추출
    ...
    문자열 ".NET"이 HTML에서 발생하는 횟수를 계산합니다.  
