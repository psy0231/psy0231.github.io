...
- Prescriptive: Naming Convention
  ...
  - Method Names and Instance Variables
    ...
    Python의 Name Mangling 규칙을 호출.
      - Name Mangling
        ...
        프로그램에서 함수나 변수를 선언했을 때,  
        선언시 사용했던 이름을  
        컴파일러가 컴파일 단계에서  
        일정한 규칙을 가지고 변형하는 것

        예시로는 Java에서 Overloading으로  
        오버로딩 시 같은 함수명을  
        컴파일러가 구분할 수 있는 이유는  
        인자의 형식과 갯수가 다를 경우  
        네임 맹글링을 통해 함수나 변수명 뒤에  
        추가 정보가 붙어 구별이 되기 때문.

        C/C++을 혼합해서 쓸 때 extern "C"도  
        이때문이라함.

    - Python은 이러한 이름을 클래스 이름으로 Mangling.  
  ...
  - Designing for Inheritance
    ...
    이러한 클래스를 디자인할 때 어떤 속성이 
    공개인지, 하위 클래스 API의 일부인지,  
    기본 클래스에서만 사용해야 하는지  
    명시적으로 결정한다.
