...
- Serilog는 사용 못한다고 치고.. NLog를 찾기로함. 여기서 안되면 포기.  
  ...
  - 근데 나랑 같은 문제로 NLog에 issue남긴걸 봤는데[^2] 저 세 뭉탱이가 다 관련되어 보임. 정확히 같은 이유로 같은 구조로 만들길 원했고 이슈 닫은걸로보아 해결한것같음... 
...
## 참고
 ...
 [^2]: [File target](https://github.com/nlog/nlog/wiki/File-target#per-level-log-files)  
 [^2]: [EventProperties Layout Renderer](https://github.com/nlog/nlog/wiki/EventProperties-Layout-Renderer)  
